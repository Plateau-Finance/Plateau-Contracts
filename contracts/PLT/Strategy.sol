...
contract StrategyMasterChefLP is StratManager {
    ...
    address public output = address(0xeCfE536a209e405Db19887830b366E397f5B917a);
    address public want;
    ...
    address public masterchef = address(0x96634DbA8Ea2d9cF2a9B5F6Ee1eCE60900E89F2a);
  ...
  function changePerformanceFee(uint256 _newFee) public onlyOwner{
    ...
    function chargeFees() internal {                   
        ...
        IJoeRouter02(unirouter).swapExactTokensForTokensSupportingFeeOnTransferTokens(toWrapped, 0, 
        outputToWrappedRoute, address(this), now);

        uint256 wrappedBal = IERC20(wrapped).balanceOf(address(this));                   
        IERC20(wrapped).safeTransfer(msg.sender, wrappedBal);
    ...
    function addLiquidity() internal {
        ...
        if (lpToken0 != output) {
            IJoeRouter02(unirouter).swapExactTokensForTokensSupportingFeeOnTransferTokens(outputHalf, 0, outputToLp0Route, address(this), now);
        }

        if (lpToken1 != output) {
            IJoeRouter02(unirouter).swapExactTokensForTokensSupportingFeeOnTransferTokens(outputHalf, 0, outputToLp1Route, address(this), now);
        }
    ...
    function makeCustomTxn(address _fromToken, address _toToken, address _unirouter, uint256 _amount) external {
        require(msg.sender == owner(), "!auth for custom txn");

        approveTxnIfNeeded(_fromToken, _unirouter, _amount);

        customPath = [_fromToken, _toToken];

        IJoeRouter02(_unirouter).swapExactTokensForTokensSupportingFeeOnTransferTokens(_amount, 0, customPath, address(this), now.add(600));
    }
