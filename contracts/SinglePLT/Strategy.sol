...
abstract contract FeeManager {
    ...
    uint256 public constant WITHDRAW_FEE = 100;  //0.1%  (amount *withdrawalFee/WITHDRAWAL_MAX)

    uint256 public constant MAX_FEE = 1000;

    uint256 public constant CALL_FEE = 50;  //0.5%  (amount * callFee/MAX_FEE)

    uint256 public constant STRATEGIST_FEE = 50; //0.5%  (amount * STRATEGIST_FEE/MAX_FEE)

    uint256 public constant FEE_BATCH = 300; //3%  (amount * feebatch/MAX_FEE)
...
contract PLTStrategy is Ownable, Pausable, FeeManager{
    ...
    constructor(address _want,
       ...
       public {
        ...
        IERC20(want).safeApprove(router, uint(-1));
        IERC20(wrapped).safeApprove(router, uint(-1));
    ...
    function deposit() public whenNotPaused {
      ...
      function setPLTFeeRecipient(address _feeBatchAddress) external onlyOwner {
        feeBatchAddress = _feeBatchAddress;
    }
  ...
  function beforeDeposit() external virtual {
        IMasterChef(masterchef).deposit(0);
        uint256 wantBal = IERC20(want).balanceOf(address(this));
        if (wantBal > 0) {
            chargeFees();
            deposit();
    ...
    function chargeFees() internal {
        ...
        uint256 toWrapped = IERC20(output).balanceOf(address(this)).mul(100).div(MAX_FEE);
        ...
        uint256 feeBatchAmount = wrappedBal.mul(FEE_BATCH).div(MAX_FEE);
    ...
    function balanceOfPool() public view returns (uint256) {
        (uint256 _amount, ) = IMasterChef(masterchef).userInfo( address(this));
        return _amount;
    }
    ...
    function pause() public onlyOwner {
        ...
        IERC20(wrapped).safeApprove(router, 0);
